...
node --test                                   # 1995 gates
```

Use the single Node.js version in `.node-version` (26.8.1, the latest Current release
verified on September 6, 2026). CI and hosted pump workflows read that same file;
Node 20/24 are no longer support targets. Update the pin together with runtime
verification when adopting a newer release, rather than letting a run float silently.
...
| `src/bootstrap-deadlock.mjs` | Pure issue #80 tracer nets (hosted Draft pump observation: as written, as shipped, seeded control), content-addressed, plus a fail-closed reading of an IX Petri analysis into `BOOTSTRAP_DEADLOCK`, `REACHABLE_DEADLOCK`, `NO_DEADLOCK` or `UNDECIDED`; enumerates nothing itself. |
...
| `tests/` | 1995 `node:test` gates, counted as top-level `test()` declarations. `node --test`; data-driven cases run inside a declaration, so the runner reports more executed cases than there are declarations. |
