...
content-addressed facts, and every derived number is a function of those facts alone.
...
A head generation is opened by `DRAFT_OPENED` and by each `HEAD_ADVANCED`. Metrics named
`*_CURRENT_HEAD` read only facts bound to the current head. A green check on a superseded head is
retained as history and can never satisfy the current head; when the current head has no green
check the metric is `UNKNOWN(NO_GREEN_CHECK_ON_CURRENT_HEAD)`.
...
Each is published as a named fact with its own value or its own unknown reason. R0 publishes no
composite, weighted, or normalized score, and no metric triggers an effect.
